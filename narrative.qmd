...
```{=html}
<iframe src="https://dyerlabteaching.github.io/Markdown/slides_classic.html#/title-slide"></iframe>
```

> Markdown & Quarto provide interactive documents for more effective data communication, tight linking between the raw data, your analyses, and your interpretation of the results, and a more efficient way to increase the reproducibility of your work—the closer we keep our data, analyses, inferences, and interpretations, the more reliable our research becomes.



[![Click to view slides](media/slides-preview.png){fig-alt="Markdown presentation slides" style="border: 1px solid #ccc; cursor: pointer;"}](https://dyerlabteaching.github.io/Markdown/slides.html#/title-slide)

<p style="text-align: center; margin-top: 0.5em;">
  <a href="https://dyerlabteaching.github.io/Markdown/slides.html#/title-slide" target="_blank">Open slides in fullscreen ↗</a>
</p>
...
This is critical for us because Code is just text. Once it is evaluated, it can be replaced with:
...
Here is a YAML from a research manuscript I'm working on so you can see the extra stuff that can be added.  
...
abstract: |
&nbsp;&nbsp;&nbsp;This manuscript develops a novel approach to estimating
&nbsp;&nbsp;&nbsp;relative directionality in genetic connectivity using a
...
&nbsp;&nbsp;&nbsp;<mark>populations of X and another is applied to an analysis of</mark>
...
What Markdown does is allow you to type these components and use 'marking' around the elements to make them different from regular text. It is really, amazingly simple.
...
Individual paragraphs are delimited by either a blank line between them or two spaces at the end of the sentence.  <mark>I find it much easier to just put the</mark>
...
One of the strengths of Quarto and its integration of `R` and markdown is the ability to mix code and text together in one place. This allows us to bring all of our analyses *and* data as close to one another as possible, helping with reproducibility and error reduction.  You do not have to do your analysis in one program, copy-and-paste the results to your word processor, then copy and paste to your presentation software, then go to your graphics program and make a figure and copy-and-paste it together, etc.  It all lives in one place.
...
Each chunk is enclosed within boundary rows, the top row **must** contain three grave accents (back ticks - \`) followed by the letter r in curly brackets \`\`\`{r}. This back tick is the grave (&grave;) character and **not a single quote** (on the US English keyboard it is the key in the top left corner).  
...
:::{.callout-warning}
If you mess up the leading and trailing lines of a chunk, the rendering process will get totally hosed in both the IDE (it does not know what to style as text versus code parts) and the rendering (same reasons).  
:::
...
There are several options we can apply to a chunk.  These can be set at the top of the document, in the YAML, which will impact all the chunks in the document, or on a per-chunk basis.  Here some options you will commonly use.
...
To put these in an individual chunk, place them *at the top of the chunk* (this is important, they should be the first lines of text) preceded by `#| `.  For example, the following chunk will show its contents, not evaluate the code, and suppress any warnings or errors when it does execute.
...
- Evaluated R Code: Any code between `r` and the ending \` will be evaluated as R code within the text. <mark>When you render/preview the document, it will be run and the contents between these symbols and replaced the content by the output of the `R` code itself.</mark>[This is HUGE!  Think of this as how you will **never** have to update numerical values in your manuscripts again if they come from your analyses.]{.aside}  So, the markdown <tt>&grave;r pi&grave;</tt> will be replaced in the text as `r pi` as if you actually wrote out the digits of $pi$.
