...
Individual paragraphs are delimited by either a blank line between them or two spaces at the end of the sentence.  
...
- Evaluated R Code: Any code between `r` and the ending \` will be evaluated as R code within the text. When you render/preview the document, it will be run from top to bottom and the contents between these code fences will be replaced by the content by the output of the `R` code itself.[This is HUGE!  Think of this as how you will **never** have to update numerical values in your manuscripts again if they come from your analyses.]{.aside}  So, the markdown <tt>&grave;r pi&grave;</tt> will be replaced in the text as `r pi` as if you actually wrote out the digits of $pi$. 
