...
A normal markdown document is the most basic kind of document available in RStudio. For this class, we will be using a slightly different kind of document, a Quarto Document (as described below) because they have a few more fancy options that make them desirable for our use. However, basic markdown is basic markdown so it is good to see what how these base level documents are created and manipulated.
...
For most of this class, we will be using HTML output. This creates a stand-alone document, with all the content embedded within it as a self-contained HTML web page. This is preferable at this point because it we will be working with interactive graphics, included maps, overlays, graphics, dynamic data tables, etc.

None of these components can be embedded within PDF or Word documents, though you could take a picture of them and use that... But that would defeat the purpose and utility of interactive graphics and output now wouldn't it?

The raw document in RStudio looks like this:

![Base R Markdown](https://live.staticflickr.com/65535/50159564677_b1839ba410_c_d.jpg) There are a couple different parts to this document itself that need to be highlighted.
...
The lastest versions of RStudio have also included a "Visual Editor" that makes the markdown a bit more like a word processor. You can switch between the source view (what is shown in the previous image) and the "Visual" version of it by selecting the "Visual" button on the upper left corner of the editor pane.

![RStudio visual editor attempting to make the markdown more 'word processor' like.](https://live.staticflickr.com/65535/52096821949_61f0bbbd39_c_d.jpg)

It is up to your preferences as to whether you like the source view or the visual view. The new editor is still a bit green so in some cases it can be a bit of a pain. Remember that if you run into a situation where it is not doing what you want it to do, switch over to the source view and give it a shot from that perspective. They both work on the same document but sometimes a raw version is easier to do what you want to.
...
An `R Notebook` is a special kind of Markdown document with just *a bit* of interactivity to the normal Markdown documents. The differences are mostly
...
We will not be using notebooks in the class either.

## Quarto Documents

[Quarto](https://quarto.org) is an open source scientific publishing system that is just one step beyond what normal Markdown does in R.
...
A quarto document is similar to the markdown one, with the YAML meta data at the top and the same use of markdown in the body of the document. To create a quarto document you need to first install it on your computer. There are versions for various operating systems at [quarto.org](https://quarto.org)

![](https://live.staticflickr.com/65535/52096855069_acbaaecd65_c_d.jpg)

Follow the instructions in Step 2 and using RStudio and it will walk you through some more examples using some cute penguin data. There is no need to reproduce that here.
